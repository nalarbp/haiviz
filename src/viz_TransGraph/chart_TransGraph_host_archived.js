...
import { line, curveNatural, pie, arc } from "d3-shape";
...
const TransGraph_host = (props) => {
  ...
  useEffect(() => {
    ...
    if (nodeSize) {
      if (props.selectedData && props.selectedData.length > 0) {
        svg
          .selectAll(".trans-pieGroup")
          .attr(
            "transform",
            (d) =>
              "translate(" + d.x + "," + d.y + ")" + "scale(" + nodeSize + ")"
          );
        updateBySelectedData();
      } else {
        svg
  ...
  useEffect(() => {
    ...
    if (props.colorScale.colorType) {
      svg.selectAll(".trans_nodeCircle").attr("fill", "black");
    }
  }, [props.colorScale]);
  useEffect(() => {
  ...
  function draw() {
    ...
    linksGroup
      ...
      .attr("font-size", () => textSize * 0.75)
      .attr("fill", "red")
      .text((d) => (d.weight ? d.weight.toFixed(2) : d.weight))
      .style("opacity", () => (isLinkLabelShown ? 1 : 0));
    ...
    // ==================== PIE CHART ======================
    const isolateDataMapByHost = group(
      isolateDataCloned,
    ...
    transmission.nodes.forEach((d) => {
      let data = null;
      if (d.name) {
        //console.log(d.name, isolateDataMapByHost);
        let isolate_data = isolateDataMapByHost.get(d.name);
        data = isolate_data ? isolate_data : null;
    ...
    let pieGroup = nodesGroup
      ...
      .enter()
      .append("g")
      .attr("class", "trans-pieGroup")
      .attr("transform", (d) => "translate(" + d.x + "," + d.y + ")");

    let pieArcGroup = pieGroup
      .selectAll(".trans-pieArcGroup")
      .data((d) => {
        return d.data ? pieGenerator(d.data) : [null];
      })
      .enter()
      .append("g")
      .attr("class", "trans-pieArcGroup");
  ...
  function updateBySelectedData() {
    svg.selectAll(".trans-pieArcGroup").style("opacity", (d) => {
      if (props.selectedData.indexOf(d.data[1][0].isolate_name) !== -1) {
        return 1;
      } else {
        return 0.2;
      }
    ...
    svg.selectAll(".trans_link").style("opacity", (d) => {
      if (d.source.data) {
        let obj = d.source.data[0][1][0];
        let objName = obj ? obj.isolate_name : null;
        if (objName) {
          if (
            props.selectedData.indexOf(objName) !== -1 ||
            props.selectedData.indexOf(objName) !== -1
          ) {
            return 1;
          } else {
  ...
  function clearSelectedData() {
    svg.selectAll(".trans-pieArcGroup").style("opacity", () => 1);
    svg.selectAll(".trans_link").style("opacity", () => 1);
    svg
      .selectAll(".trans_linksLabel")
      .style("opacity", () => (isLinkLabelShown ? 1 : 0));
  }
  ...
  return (
    <React.Fragment>
      <div id="transmissionContainer" ref={transmissionContainerRef}>
        ...
        <div id="transmission-tooltip"></div>
